...
updatedAt: 2026-09-24T17:00:03Z
...
> [!WARNING]
> We _can_ instead define an exact semver specifier for `pydantic==2.13.5`, but that doesn't _lock_ the versions of its transitive dependencies. Nothing stops `uv` from installing newer versions of semver-compatible transitive dependencies during the next `uv run` in a different sandbox session.
